...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Проверяем, есть ли облачный пароль у аккаунта
    // В реальном проекте здесь должен быть запрос к Telegram API или TDLib
    // Сейчас мы эмулируем: для большинства номеров пароля нет, но иногда есть
    const hasPassword = Math.random() < 0.2; // 20% шанс, что есть пароль (для демо)

    // Генерируем сессионные данные
    const sessionData = {
